...
uint32_t bcm2835_peri_read(volatile uint32_t* paddr)
{
    ...
    if (debug)
    {
        rtdm_printk("bcm2835_peri_read  paddr %08X\n", (unsigned) paddr);
	return 0;
    }
    else
    {
...
uint32_t bcm2835_peri_read_nb(volatile uint32_t* paddr)
{
    if (debug)
    {
	rtdm_printk("bcm2835_peri_read_nb  paddr %08X\n", (unsigned) paddr);
	return 0;
    }
    else
    {
...
void bcm2835_peri_write(volatile uint32_t* paddr, uint32_t value)
{
    if (debug)
    {
	rtdm_printk("bcm2835_peri_write paddr %08X, value %08X\n", (unsigned) paddr, value);
    }
    else
    {
        __sync_synchronize();
...
void bcm2835_peri_write_nb(volatile uint32_t* paddr, uint32_t value)
{
    if (debug)
    {
	rtdm_printk("bcm2835_peri_write_nb paddr %08X, value %08X\n",
               (unsigned) paddr, value);
    }
    else
    {
...
void bcm2835_delayMicroseconds(uint64_t micros)
{
	...
	rtdm_printk("bcm2835_delayMicroseconds %lld\n", micros);
	return;
    }
...
static void *mapmem(const char *msg, size_t size, off_t off)
{
	...
	if (map < 0)
	rtdm_printk( "%s: mapping 0x%08lx for %s failed\n", __FUNCTION__, off, msg);
	else
	rtdm_printk( "%s: mapping 0x%08lx for %s succeded to 0x%p\n", __FUNCTION__, off, msg, map);
	return map;
}
